...
while True:
    ...
    # Keypoints are reselected per 40 frames.
    if frame_counter % 40 == 0:
        ...
        # Convert the selected points into a numpy array and reshape it
        if len(points_to_track) > 0:
            new_p1 = np.array(points_to_track, dtype=np.float32).reshape(-1, 1, 2)
        else:
            print("No points selected!")
            exit()
    else:
...
generate_tracking_mask(points_to_mask, (old_frame.shape[0], old_frame.shape[1]), "./mask_mannual_reselect")
